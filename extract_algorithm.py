...
from similar_word.find_related_words import load_file
    ...
    for index, sentence in enumerate(cut_sents):
        ...
        if ('S-Nh' in ner_tag) or ('S-Ni' in ner_tag) or ('S-Ns' in ner_tag):
            for i, p in enumerate(parser):
                if (p[1] == 'SBV') and word[p[0] - 1] in say_word:  
                    person = word[i]
                    say = word[p[0] - 1]
...
if __name__ == '__main__':
    ...
    '''
    short_test = '小明表示"这块土地是他承包的。"土地上的苹果也是他的。小芳表示她很认同。'

    print('测试文本：',test_string)
